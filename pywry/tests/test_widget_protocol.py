...
# =============================================================================
# NativeWindowHandle Notification and Toolbar State Tests
# =============================================================================


class TestNativeWindowHandleAlertAndToolbar:
    """Tests for NativeWindowHandle alert and toolbar state methods."""

    def test_alert_emits_pywry_alert_event(self, native_handle, mock_app):
        """Test alert() emits pywry:alert with correct payload."""
        native_handle.alert("File saved successfully")
        mock_app.emit.assert_called_once_with(
            "pywry:alert",
            {"message": "File saved successfully", "type": "info", "position": "top-right"},
            "test-window",
        )

    def test_alert_with_type_and_title(self, native_handle, mock_app):
        """Test alert() with success type and title."""
        native_handle.alert("Export complete", alert_type="success", title="Done")
        mock_app.emit.assert_called_once_with(
            "pywry:alert",
            {
    ...
    def test_alert_with_duration(self, native_handle, mock_app):
        """Test alert() with custom duration."""
        native_handle.alert("Warning", alert_type="warning", duration=8000)
        call_args = mock_app.emit.call_args
        assert call_args[0][1]["duration"] == 8000
        assert call_args[0][1]["type"] == "warning"

    def test_alert_with_callback_event(self, native_handle, mock_app):
        """Test alert() confirm type with callback event."""
        native_handle.alert(
            "Are you sure?",
            alert_type="confirm",
            callback_event="app:confirm",
        )
        call_args = mock_app.emit.call_args
        assert call_args[0][1]["type"] == "confirm"
        assert call_args[0][1]["callback_event"] == "app:confirm"

    def test_alert_with_position(self, native_handle, mock_app):
        """Test alert() with custom position."""
        native_handle.alert("Notice", position="bottom-left")
        call_args = mock_app.emit.call_args
        assert call_args[0][1]["position"] == "bottom-left"

    def test_alert_optional_fields_not_included_when_none(self, native_handle, mock_app):
        """Test alert() omits None optional fields from payload."""
        native_handle.alert("Simple message")
        payload = mock_app.emit.call_args[0][1]
        assert "title" not in payload
        assert "duration" not in payload
        assert "callback_event" not in payload
    ...
    def test_request_toolbar_state_with_toolbar_id(self, native_handle, mock_app):
        ...
        native_handle.request_toolbar_state(toolbar_id="toolbar-abc123")
        mock_app.emit.assert_called_once_with(
            "toolbar:request-state",
            {"toolbarId": "toolbar-abc123"},
            "test-window",
        )

    def test_set_toolbar_value_with_value(self, native_handle, mock_app):
        """Test set_toolbar_value() emits toolbar:set-value with value."""
        native_handle.set_toolbar_value("theme-select", "light")
        mock_app.emit.assert_called_once_with(
            "toolbar:set-value",
            {"componentId": "theme-select", "value": "light"},
            "test-window",
        )

    def test_set_toolbar_value_without_value(self, native_handle, mock_app):
        """Test set_toolbar_value() without value only sets other attrs."""
        native_handle.set_toolbar_value("submit-btn", disabled=True, label="Loading...")
        call_args = mock_app.emit.call_args
        assert call_args[0][0] == "toolbar:set-value"
        payload = call_args[0][1]
        assert payload["componentId"] == "submit-btn"
        assert payload["disabled"] is True
        assert payload["label"] == "Loading..."
        assert "value" not in payload

    def test_set_toolbar_value_with_toolbar_id(self, native_handle, mock_app):
        """Test set_toolbar_value() passes toolbar_id."""
        native_handle.set_toolbar_value("slider", 50, toolbar_id="toolbar-abc")
        payload = mock_app.emit.call_args[0][1]
        assert payload["toolbarId"] == "toolbar-abc"

    def test_set_toolbar_value_none_value_not_included(self, native_handle, mock_app):
        """Test set_toolbar_value() with no value doesn't include it in payload."""
        native_handle.set_toolbar_value("zoom-input", label="Zoom")
        payload = mock_app.emit.call_args[0][1]
        assert "value" not in payload
        assert payload["label"] == "Zoom"

    def test_set_toolbar_values_emits_event(self, native_handle, mock_app):
        """Test set_toolbar_values() emits toolbar:set-values."""
        values = {"theme-select": "light", "zoom-input": 100}
        native_handle.set_toolbar_values(values)
        mock_app.emit.assert_called_once_with(
            "toolbar:set-values",
            {"values": values},
            "test-window",
    ...
    def test_set_toolbar_values_with_toolbar_id(self, native_handle, mock_app):
        """Test set_toolbar_values() passes toolbar_id."""
        native_handle.set_toolbar_values({"slider": 50}, toolbar_id="toolbar-abc")
        payload = mock_app.emit.call_args[0][1]
        assert payload["toolbarId"] == "toolbar-abc"
        assert payload["values"] == {"slider": 50}
