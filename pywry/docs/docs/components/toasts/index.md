...
!!! info "Why not native OS notifications?"

    PyWry implements toast notifications as an in-window HTML/CSS component rather
    than delegating to Tauri's native notification API or the operating system.
    This is intentional — the in-window toast is guaranteed to look and behave
    identically across **all rendering paths** (native desktop, Jupyter widget,
    browser tab) and **all operating modes** (inline, deploy, headless). Native OS
    notifications would only work in the desktop path and would require separate
    permission handling on each platform.
...
- **[Modals](../modal/index.md)** — Blocking dialogs with form components
- **[Toolbar System](../toolbar/index.md)** — Interactive components that trigger toasts
- **[Event System](../../guides/events.md)** — How `pywry:alert` events are dispatched
